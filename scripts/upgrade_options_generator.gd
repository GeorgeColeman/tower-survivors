...
func generate_upgrade_options(game: Game, amount: int) -> UpgradeOptions:
	...
	if GameRules.CORE_AS_UPGRADE_OPTION:
		options.append(UpgradeOptionFactory.add_core(game.player))
...
func _get_tower_options(game: Game) -> Array[UpgradeOption]:
	...
	options.append(UpgradeOptionFactory.rank_up_specific_tower(game.tower))
	...
	for tower_resource: TowerResource in game.game_data.tower_resource_dict.values():
		...
		options.append(UpgradeOptionFactory.new_tower(
			tower_resource,
			func():
				game.building_options.add_building_option(tower_resource, game.player)
		))
	...
	for passive_key in game.game_data.passives_dict.keys():
		...
		if game.player.upgrades.passives_dict.has(passive_key):
			if game.player.upgrades.can_rank_up_passive(passive_key):
				options.append(UpgradeOptionFactory.rank_up_passive(passive_key, game.player))
		else:
			options.append(UpgradeOptionFactory.new_passive(passive, game.player))
